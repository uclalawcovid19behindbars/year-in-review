...
## TO DO : add a rates version of this plot, using this: 
## https://github.com/themarshallproject/COVID_prison_data/blob/master/data/staff_populations.csv
mp_dat <- behindbarstools::read_mpap_data()

spine_plot_cases <- long_counts %>%
  filter(metric %in% c("Residents.Confirmed", "Staff.Confirmed"),
         !is.na(count)) %>%
  mutate(plot_count = ifelse(metric == "Staff.Confirmed", -1*count, count)) %>% 
  mutate(state_short = behindbarstools::translate_state(State, reverse = TRUE),
         state_short = ifelse(is.na(state_short), State, state_short)) %>%
  ggplot(data=., aes(
                      x = reorder(state_short, plot_count),
                      # x = state_short,
                      y = plot_count, 
                      fill = metric
                     )) +
  geom_bar(stat="identity") + 
  # geom_text(aes(x=state_short,
  #               y=plot_count,
  #               label=abs(plot_count)),
  #               vjust = ifelse(plot_count >= 0, 0, 1)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) + 
