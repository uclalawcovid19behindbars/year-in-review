...
res_cases_dat <- filled_dates %>%
  ...
  mutate(countReal = rowSums(!is.na(select(., -State, -Name, -Jurisdiction, -NameToShow, -highlightFac)))) %>%
  filter(countReal > 90) %>% # only keep data we have values for more than 3 months
  select(NameToShow, highlightFac, `2020-03-26`:`2021-04-05`) 
